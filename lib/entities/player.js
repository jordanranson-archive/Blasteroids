global.Player = global.Entity.extend({
    ...
    update: function( time, input, socket ) {
        ...
        if( this.inputState.forward ) {
            this.vel.x += (Math.cos(Math.radians(this.angle))*this.speed) * (delta/15);
            this.vel.y += (Math.sin(Math.radians(this.angle))*this.speed) * (delta/15);
        }

        // Move backward
        if( this.inputState.backward ) {
            this.vel.x -= (Math.cos(Math.radians(this.angle))*(this.speed*.6)) * (delta/15);
            this.vel.y -= (Math.sin(Math.radians(this.angle))*(this.speed*.6)) * (delta/15);
        }
